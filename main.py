...
def run_simulation(time, dataset, algo, model, similarity, alpha, beta, number, dim_input, dim_output, same_sample_size,
    ...
    local_learning_rate_list = [30.0, 50.0, 100.0]
    if dp == "Gaussian":
        local_learning_rate_list = [0.03, 0.1, 0.3, 1.0]
    ...
    if generate:
        ...
        if len(list_of_local_updates) == len(list_of_sigma_gaussian) and len(list_of_sigma_gaussian) > 0:
            ...
            plot_train_dissimilarity(dataset, algorithms, noises, similarities, str(number), sigma_gaussian,
                                     local_updates, sample_ratio, input_dict["model"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_glob_iters", type=int, default=250, help="Number of communication rounds")

    parser.add_argument("--dataset", type=str, default="Femnist", choices=["Femnist", "Logistic", "CIFAR_10", "Mnist"])
    parser.add_argument("--algo", type=str, default="FedAvg", choices=["FedSGD", "FedAvg", "SCAFFOLD-warm", "SCAFFOLD"])
    parser.add_argument("--model", type=str, default="NN1_PCA", choices=["mclr", "NN1", "NN1_PCA", "CNN"])
    ...
    parser.add_argument("--generate_pca", type=int, default=0,
                        help="True to generate data with PCA (not Logistic data)")
    parser.add_argument("--dim_pca", type=int, default=60,
                        help="Nb of components for generate_pca")
